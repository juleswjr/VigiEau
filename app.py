...
with st.expander("✏️ Modifier la liste des adresses", expanded=False):
    adresses_txt = st.text_area(
        "Une adresse par ligne",
        value=ADRESSES_DEFAULT,
        height=300
    )
...
if st.button("🔍 Lancer l'analyse VigiEau", type="primary", use_container_width=True):
    ...
    for i, adresse in enumerate(adresses):
        ...
        row = {"Adresse": adresse, "Niveau de gravité": "NA", "Types d'eau": ""}
    ...
    progress.empty()
    df = pd.DataFrame(rows, columns=["Adresse", "Niveau de gravité", "Types d'eau"])

    # ── Résumé ────────────────────────────────────────────────────────────
    nb_restr = df[~df["Niveau de gravité"].isin(["NA", ""])
                  & ~df["Niveau de gravité"].str.startswith("⚠️")].shape[0]
    col1, col2, col3 = st.columns(3)
    col1.metric("Sites analysés", len(df))
    col2.metric("Sites avec restriction", nb_restr)
    col3.metric("Sites sans restriction", len(df) - nb_restr)
    ...
    st.dataframe(
        df.style.apply(colorize_row, axis=1),
